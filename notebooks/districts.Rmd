...
```{r}
districts |> 
  filter(alg_1 <= 50.00 | eng_1 <= 50.0)
```
8 districts have a proficiency rate less than 50% for either eng_1 or alg_1. 
...
13. Read in the school-level testing data for 2014, available. You might find the readxl library useful for this task. If you use this library, be sure to look at the `na` argument for the `read_excel` function.
New link --> https://docs.google.com/spreadsheets/d/1i6s9wPs5lg9fFqadCfW9qccPwuKSIu2t/edit?usp=sharing&ouid=102586165284403190694&rtpof=true&sd=true, dont use the other one 
